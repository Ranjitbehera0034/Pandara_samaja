...
export default function Leaders() {
    ...
    return (
        <div className="p-8 pb-32 max-w-7xl mx-auto dark:text-slate-200">
                        ...
                        {filteredLeaders.map(leader => (
                            <div key={leader.id} className="relative group bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300">
                                <div className="aspect-[4/3] bg-slate-100 dark:bg-slate-900 relative">
                                    {leader.image_url ? (
                                        <img src={leader.image_url.startsWith('http') || leader.image_url.startsWith('blob:') ? leader.image_url : (leader.image_url.startsWith('assets/') ? `http://localhost:3000/${leader.image_url}` : `${(typeof window !== 'undefined' && (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1')) ? 'http://localhost:5000' : 'https://pandara-samaja-backend.onrender.com'}/${leader.image_url.startsWith('/') ? '' : '/'}${leader.image_url}`)} alt={leader.name} className="w-full h-full object-cover" />
                                    ) : (
                                ...
                                >
                                    {imagePreview ? (
                                        <div className="relative w-full h-full rounded-lg overflow-hidden">
                                            <img src={imagePreview.startsWith('http') || imagePreview.startsWith('blob:') ? imagePreview : (imagePreview.startsWith('assets/') ? `http://localhost:3000/${imagePreview}` : `${(typeof window !== 'undefined' && (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1')) ? 'http://localhost:5000' : 'https://pandara-samaja-backend.onrender.com'}/${imagePreview.startsWith('/') ? '' : '/'}${imagePreview}`)} alt="Preview" className="w-full h-full object-contain" />
